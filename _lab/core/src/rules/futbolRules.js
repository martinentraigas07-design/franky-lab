...
export function tickFutbolRules(world, robotId, dtMs, robotSpeedCmS, robotHeadingRad) {
    if (!world.match.started || world.match.ended)
        return; // esperando "Iniciar Partido" — nadie se mueve hasta entonces (ver sw-entry.ts)
    world.match.elapsedMs += dtMs;
    if (world.match.durationMs !== null && world.match.elapsedMs >= world.match.durationMs) {
        const winner = world.match.scoreRobot > world.match.scoreOponente ? "robot"
    ...
    if (ball) {
        for (const goal of world.goals) {
            if (ballFullyInGoal(ball, goal)) {
                if (goal.team === "derecho")
                    world.match.scoreRobot++; // FRANKY ataca el arco derecho
                else
                    world.match.scoreOponente++; // el oponente ataca el arco izquierdo (el "propio" de FRANKY)
...
function robotPositionOf(world, robotId) {
    const r = world.robots[robotId];
    return r ? r.position : { x: 0, y: 0 };
}
function ballFullyInGoal(ball, goal) {
