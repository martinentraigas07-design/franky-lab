 ...
 */
import { endMatch, startMatch } from "../worldModel.js";
import { isOutsideDohyo } from "../simulationEngine.js";
export function tickMinisumoRules(world, robotId) {
    if (world.arena.shape !== "circle")
        return;
    if (!world.match.started)
        startMatch(world); // Minisumo no usa botón manual: arranca con el modo (ver sw-entry.ts)
    if (world.match.ended)
        return;
