 ...
 */
import { createDefaultWorld, getRobot, addObstacle, moveObstacle, removeObstacle, addWall, removeWall, resetScenario, loadScenario, advanceClock, setAnalogPin, setRobotPose, setOpponentPose, impulseBall, startMatch, } from "../../core/src/worldModel.js";
...
startServiceWorker(server, [...API_ROUTES, ...LAB_ROUTES], (dtMs) => {
    ...
    const sensorConfig = server.getActiveSensorConfig ? server.getActiveSensorConfig() : null;
    if (sensorConfig) {
        ...
        if (sensorConfig.tipo === 2 /* SHARP */) {
            ...
            if (sensorConfig.numDist >= 2)
                robot.pins.analog[sensorConfig.sharpPinD] = distanceToSharpADC(distDer, 80);
        }
            ...
            if (sensorConfig.numDist >= 2)
                robot.pins.analog[sensorConfig.echoD] = Math.round(distDer);
        }
    ...
    const mode = server.getLiveState ? server.getLiveState().mode : undefined;
    ...
    if (buttonJustPressed) {
        ...
        else if ((world.scenarioId === "futbol" || world.scenarioId === "laberinto" || world.scenarioId === "linea") &&
            !world.match.started) {
...
server.getLiveState = () => {
    ...
    return {
        ...
        retardoOK: (server.getCombatTiming ? server.getCombatTiming().retardoOK : true),
        ...
        match: { ...world.match },
        lineSensors: world.scenarioId === "linea"
            ? {
    ...
    switch (data?.type) {
        case "FRANKY_LAB_SET_ROBOT_POSE":
            if (data.x !== undefined && data.y !== undefined)
                setRobotPose(world, ROBOT_ID, data.x, data.y, data.headingRad);
            break;
        case "FRANKY_LAB_SET_OPPONENT_POSE":
            if (data.id && data.x !== undefined && data.y !== undefined)
                setOpponentPose(world, data.id, data.x, data.y, data.headingRad);
            break;
        case "FRANKY_LAB_IMPULSE_BALL":
            if (data.id && data.x !== undefined && data.y !== undefined)
                impulseBall(world, data.id, data.x, data.y);
            break;
        ...
        case "FRANKY_LAB_MOVE_OBSTACLE":
            if (data.id && data.x !== undefined && data.y !== undefined)
                moveObstacle(world, data.id, data.x, data.y);
            break;
        case "FRANKY_LAB_REMOVE_OBSTACLE":
            if (data.id)
                removeObstacle(world, data.id);
            break;
        ...
        case "FRANKY_LAB_REMOVE_WALL":
            if (data.id)
                removeWall(world, data.id);
            break;
        case "FRANKY_LAB_LOAD_SCENARIO":
            if (data.scenarioId)
                loadScenario(world, ROBOT_ID, data.scenarioId);
            break;
        ...
        case "FRANKY_LAB_SET_ANALOG_INPUT":
            ...
            if (data.pin !== undefined && data.value !== undefined)
                setAnalogPin(world, ROBOT_ID, data.pin, data.value);
            break;
    }
});
