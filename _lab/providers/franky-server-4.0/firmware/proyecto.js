 ...
 */
import { CABECEO_MS } from "./model.js";
...
function leerEntero(obj, key, min, max) {
    const v = obj[key];
    if (typeof v !== "number" || !Number.isFinite(v))
        throw new ProyectoError(`falta o invalido: ${key}`);
    if (v < min || v > max)
        throw new ProyectoError(`fuera de rango: ${key}=${v}`);
    return v;
}
function leerBool(obj, key) {
    const v = obj[key];
    if (typeof v !== "boolean")
        throw new ProyectoError(`${key}: falta o invalido`);
    return v;
}
function leerObjeto(obj, key) {
    const v = obj[key];
    if (typeof v !== "object" || v === null || Array.isArray(v))
        throw new ProyectoError(`falta la seccion "${key}"`);
    return v;
}
function proyectoJsonSumo(cfg, completo, velExterna, velInterna) {
    const j = {
...
function validarSumo(obj, base, completo) {
    const destino = { ...base };
    destino.tipoDistSensor = leerEntero(obj, "tipoDistSensor", 0, 2);
    ...
    destino.numDistSensores = destinoNumDist; // micro es siempre 1 sensor — no se persiste distinto
    ...
    destino.sharpPinI = leerEntero(obj, "sharpPinI", 0, 1);
    if (completo)
        destino.sharpPinD = leerEntero(obj, "sharpPinD", 0, 1);
    else
        destino.sharpPinD = 0;
    destino.umbralSharp = leerEntero(obj, "umbralSharp", 100, 4095);
    destino.optPinI = leerEntero(obj, "optPinI", 0, 255);
    if (!gpioValido(destino.optPinI))
        throw new ProyectoError("optPinI: GPIO fuera del hardware real (ESP32-C3 SuperMini)");
    if (completo) {
        destino.optPinD = leerEntero(obj, "optPinD", 0, 255);
        if (!gpioValido(destino.optPinD))
            throw new ProyectoError("optPinD: GPIO fuera del hardware real (ESP32-C3 SuperMini)");
    }
    else {
        destino.optPinD = 0;
    }
    destino.numBorde = leerEntero(obj, "numBorde", 0, 2);
    destino.bordePinI = leerEntero(obj, "bordePinI", 0, 1);
    if (completo)
        destino.bordePinD = leerEntero(obj, "bordePinD", 0, 1);
    else
        destino.bordePinD = 0;
    ...
    if (completo) {
        destino.estrategia = leerEntero(obj, "estrategia", 0, 1);
...
function validarI2C(obj) {
    ...
    if (sda === scl)
        throw new ProyectoError("SDA y SCL no pueden ser el mismo GPIO");
    return { enabled, sda, scl };
}
function validarSpi(obj) {
    return { enabled: leerBool(obj, "enabled") };
}
function validarTrim(obj) {
...
function validarBlockly(obj) {
    const xml = obj["xml"];
    if (xml === null || xml === undefined)
        return null;
    if (typeof xml !== "string")
        throw new ProyectoError("blockly.xml: falta, o no es texto ni null");
    const t = xml.trim();
    if (t.length === 0)
        return null; // equivalente a "sin programa"
    if (t.length > 40000)
        throw new ProyectoError("blockly.xml: demasiado grande (>40000 caracteres)");
    if (!t.startsWith("<xml"))
        throw new ProyectoError("blockly.xml: no tiene forma de XML de Blockly (debe empezar con <xml)");
    if (!t.endsWith("</xml>") && !t.endsWith("/>"))
        throw new ProyectoError("blockly.xml: no parece un XML de Blockly bien formado");
    return xml;
}
// ---- API pública: export ----
export function generarProyectoJson(cfgMini, cfgMicro, perfilActivo, inicioServidor, inicioBoton, i2cEnabled, i2cSda, i2cScl, spiEnabled, trimA, trimB, velExterna, velInterna) {
    return {
...
export function validarProyectoImport(body, baseMini, baseMicro) {
    ...
    if (typeof version !== "number")
        throw new ProyectoError('Falta "version" en el archivo');
    ...
    if (perfilRaw === 0 || perfilRaw === 1)
        perfilActivo = perfilRaw;
    ...
    if (typeof inicioRaw === "object" && inicioRaw !== null && !Array.isArray(inicioRaw)) {
        const inicioObj = inicioRaw;
        if (typeof inicioObj["servidor"] === "boolean")
            inicioServidor = inicioObj["servidor"];
        if (typeof inicioObj["boton"] === "boolean")
            inicioBoton = inicioObj["boton"];
    }
