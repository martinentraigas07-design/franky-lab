 ...
 */
import { RobotMode, MONITOR_BUF_N, MONITOR_MSG_LEN } from "./model.js";
/** Réplica de modoTexto() real — nombre completo de cada RobotMode. */
export function modoTexto(m) {
    switch (m) {
...
export function monitorContextoActual(model) {
 ...
 */
export function frankyLog(model, hal, nivel, contexto, msg) {
    if (nivel === "D" && !model.debugEnabled)
        return;
...
export function flog(model, hal, nivel, msg) {
 ...
 */
export function generarLogTexto(model, hal) {
    let o = "";
    ...
    const start = model.monitorSeq < MONITOR_BUF_N ? 0 : model.monitorHead;
    if (total === 0)
        o += "(sin eventos registrados todavia)\n";
    for (let i = 0; i < total; i++) {
        const e = model.monitorBuf[(start + i) % MONITOR_BUF_N];
