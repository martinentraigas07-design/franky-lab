 ...
 */
import { GpioMotivo, GPIO_EXPUESTOS, TipoDistSensor } from "./model.js";
...
export function gpioIndex(pin) {
    return GPIO_EXPUESTOS.indexOf(pin);
}
export function gpioValido(pin) {
...
export function gpioReservar(model, pin, motivo) {
    const idx = gpioIndex(pin);
    if (idx < 0)
        return false; // GPIO inexistente en este hardware
    const actual = model.gpioReserva[idx];
    if (actual !== GpioMotivo.LIBRE && actual !== motivo)
        return false;
...
export function gpioLiberar(model, pin) {
    const idx = gpioIndex(pin);
    if (idx < 0)
        return;
    const m = model.gpioReserva[idx];
    if (m === GpioMotivo.MOTOR || m === GpioMotivo.LED || m === GpioMotivo.BOTON || m === GpioMotivo.ADC_FIJO)
        return;
...
export function gpioLiberarPorMotivo(model, motivo) {
    for (let i = 0; i < model.gpioReserva.length; i++) {
        if (model.gpioReserva[i] === motivo)
            model.gpioReserva[i] = GpioMotivo.LIBRE;
    }
}
export function gpioDisponible(model, pin) {
    const idx = gpioIndex(pin);
    if (idx < 0)
        return false; // un GPIO que no existe nunca está "disponible"
    return model.gpioReserva[idx] === GpioMotivo.LIBRE;
}
export function gpioMotivoActual(model, pin) {
    const idx = gpioIndex(pin);
    if (idx < 0)
        return GpioMotivo.MOTOR; // valor defensivo — igual criterio que el real (nunca debería consultarse un pin inválido)
    return model.gpioReserva[idx];
}
export function gpioMotivoTexto(m) {
    switch (m) {
 ...
 */
export function pinBloqueadoPorBus(model, pin) {
 ...
 */
export function gpioSincronizarBuses(model) {
...
export function esPinI2CValido(pin) {
...
export function aplicarI2C(model, sda, scl) {
    ...
    if (!gpioDisponible(model, sda) || !gpioDisponible(model, scl)) {
        ...
        const error = "GPIO ocupado por otra funcion (sda=" + gpioMotivoTexto(gpioMotivoActual(model, sda)) +
 ...
 * reservarlos acá.
 */
export function gpioAplicarReservasSumoActivo(model, cfgActiva) {
    ...
    if (cfgActiva.tipoDistSensor === TipoDistSensor.SONAR) {
        ...
        if (cfgActiva.numDistSensores >= 2)
            gpioReservar(model, cfgActiva.optPinD, GpioMotivo.SENSOR_SUMO);
    }
}
export function gpioEstadoSnapshot(model) {
