...
export interface ConfigureSumoInput {
  ...
  spdAtaque?: number; spdBuscExt?: number; spdBuscInt?: number; spdEvasion?: number;
...
export class FirmwareRuntime {
  ...
  configureSumo(input: ConfigureSumoInput): CommandResult {
    ...
    if (input.spdBuscExt !== undefined) cfg.spdBuscExt = constrain(input.spdBuscExt, 0, 255);
    if (input.spdBuscInt !== undefined) cfg.spdBuscInt = constrain(input.spdBuscInt, 0, 255);
    if (input.spdEvasion !== undefined) cfg.spdEvasion = constrain(input.spdEvasion, 0, 255);
    if (input.circuloExt !== undefined) cfg.spdBuscExt = constrain(input.circuloExt, 0, 255);
    if (input.circuloInt !== undefined) cfg.spdBuscInt = constrain(input.circuloInt, 0, 255);
    ...
    if (cfg.tipoDistSensor === TipoDistSensor.SONAR) {
      const conflict =
        pinBloqueadoPorBus(cfg.trigI, this.model.i2cEnabled) ||
      ...
      if (conflict) return { ok: false, error: "GPIO conflict: sonar pin used by active I2C/SPI bus" };
    }
    if (cfg.tipoDistSensor === TipoDistSensor.OPTICO) {
      const conflict =
        pinBloqueadoPorBus(cfg.optPinI, this.model.i2cEnabled) ||
        (cfg.numDistSensores >= 2 && pinBloqueadoPorBus(cfg.optPinD, this.model.i2cEnabled));
      if (conflict) return { ok: false, error: "GPIO conflict: optical pin used by active I2C/SPI bus" };
    }
  ...
  // ---- Bloques ----
  bloquesAdd(op: number, val: number, txt?: string): CommandResult {
  ...
  private tickBloques(): void {
    ...
    switch (ins.op) {
      case 1: moveDirLetter(m, this.hal, "f", ins.val > 0 ? ins.val : m.motorSpeed); m.progPC++; break; // OP_ADE
      case 2: moveDirLetter(m, this.hal, "b", ins.val > 0 ? ins.val : m.motorSpeed); m.progPC++; break; // OP_ATR
      case 3: moveDirLetter(m, this.hal, "l", ins.val > 0 ? ins.val : m.motorSpeed); m.progPC++; break; // OP_IZQ
      case 4: moveDirLetter(m, this.hal, "r", ins.val > 0 ? ins.val : m.motorSpeed); m.progPC++; break; // OP_DER
