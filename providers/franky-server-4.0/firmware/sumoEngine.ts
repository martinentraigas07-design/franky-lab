 ...
 */
import { CABECEO_MS, DireccionOponente, EvadeState, FirmwareModel, SumoConfig, TipoDistSensor } from "./model.js";
...
export function direccionTexto(d: DireccionOponente): string {
  switch (d) {
 ...
 */
export function velExterna(cfg: SumoConfig): number {
  const v = cfg.velBusqueda + Math.floor(cfg.intGiro / 2);
  return Math.max(0, Math.min(255, v));
}
export function velInterna(cfg: SumoConfig): number {
  const v = cfg.velBusqueda - Math.floor(cfg.intGiro / 2);
  return Math.max(0, Math.min(255, v));
}
...
function searchSweep(model: FirmwareModel, hal: RobotHAL, cfg: SumoConfig, now: number): void {
  ...
  if (model.cabeceoDirDer) moveDiff(model, hal, velExterna(cfg), velInterna(cfg)); // curva hacia un lado
  else moveDiff(model, hal, velInterna(cfg), velExterna(cfg)); // curva hacia el otro lado
}
...
export function ejecutarSumo(model: FirmwareModel, hal: RobotHAL, cfg: SumoConfig): void {
  ...
  if (oponentePresente) {
    ...
    moveDiff(model, hal, cfg.spdAtaque, -cfg.spdAtaque); return; // girar der (!oI && oD)
  }
  ...
  if (model.enRecuperacion) {
    if (now - model.tRecuperacion >= RECUPERACION_MS) {
      ...
      if (model.recuperacionHaciaIzquierda) moveDiff(model, hal, velInterna(cfg), velExterna(cfg));
      else moveDiff(model, hal, velExterna(cfg), velInterna(cfg));
      return;
    }
  }
 ...
 */
export function reiniciarMemoriaSumo(model: FirmwareModel): void {
