 ...
 */
import { WorldModel, getRobot, Vector2, Obstacle, OpponentRobot, Ball } from "./worldModel.js";

export interface DriveParams {
  wheelbaseCm: number;
  maxSpeedCmS: number;
  accel: number; // 0-1, qué tan rápido la velocidad real alcanza la velocidad objetivo
}

const DEFAULT_DRIVE: DriveParams = { wheelbaseCm: 11, maxSpeedCmS: 55, accel: 0.28 };
...
export function pointToSegmentDistance(p: Vector2, a: Vector2, b: Vector2): { distance: number; closest: Vector2 } {
...
function pointToBoxDistance(p: Vector2, o: Obstacle): number {
 ...
 */
export function isOverLine(world: WorldModel, point: Vector2, lineWidthCm = 2): boolean {
  const half = lineWidthCm / 2;
  for (const line of world.lines) {
    for (let i = 0; i < line.points.length - 1; i++) {
      if (pointToSegmentDistance(point, line.points[i], line.points[i + 1]).distance <= half) return true;
    }
  }
  return false;
}
...
export class DifferentialDrive {
  private vL = 0;
  private vR = 0;
  constructor(private params: DriveParams = DEFAULT_DRIVE) {}

  /** leftNorm/rightNorm en [-1, 1] (signo = sentido, magnitud = PWM normalizado). */
  step(world: WorldModel, robotId: string, leftNorm: number, rightNorm: number, dtMs: number): void {
    ...
    for (const o of world.obstacles) {
      if (obstacleDistance(next, o) >= ROBOT_RADIUS_CM) continue;
      if (!o.physics.movable) { pushBlocked = true; break; }
      const massRatio = robot.physics.massG / (robot.physics.massG + o.physics.massG);
      if (massRatio < 0.2) { pushBlocked = true; break; } // demasiado liviano para moverlo
    ...
    const blockedByWall = world.walls.some(
      (w) => pointToSegmentDistance(next, w.from, w.to).distance < ROBOT_RADIUS_CM,
    );
    ...
    for (const op of Object.values(world.opponents) as OpponentRobot[]) {
      const d = dist(next, op.position);
      if (d >= ROBOT_RADIUS_CM + op.radius) continue;
      const massRatio = robot.physics.massG / (robot.physics.massG + op.physics.massG);
      if (massRatio < 0.2) { opponentBlocked = true; break; }
  ...
  currentAngularSpeedRadS(): number {
    return (this.vL - this.vR) / this.params.wheelbaseCm;
  }
}

function obstacleDistance(p: Vector2, o: Obstacle): number {
 ...
 */
export function stepOpponent(world: WorldModel, opponentId: string, dtMs: number): void {
  ...
  if (opp.behavior.pattern === "quieto") return;
...
function stepOpponentCombat(world: WorldModel, opp: OpponentRobot, dtMs: number): void {
  const now = world.clock.simTime;
  if (opp.behavior.bornAt < 0) opp.behavior.bornAt = now;
  ...
  switch (opp.behavior.combatState) {
    ...
    case "atacando": {
      ...
      if (toRobot) opp.headingRad = angleToRobot; // embiste de frente, sin esquivar
...
function checkDohyoBorder(world: WorldModel, opp: OpponentRobot): { left: boolean; right: boolean } {
  if (world.arena.shape !== "circle") return { left: false, right: false };
...
function stepOpponentEvasion(world: WorldModel, opp: OpponentRobot, now: number): void {
...
export function moveOpponentBody(world: WorldModel, opp: OpponentRobot, headingRad: number, speedCmS: number, dtMs: number, bounceOnBlock: boolean): void {
  const dt = dtMs / 1000;
  ...
  const blockedByStatic =
    world.obstacles.some((o) => obstacleDistance(next, o) < opp.radius) ||
  ...
  for (const robot of Object.values(world.robots)) {
    ...
    if (massRatio < 0.2) { blockedByRobot = true; break; }
 ...
 */
export function stepBall(world: WorldModel, ball: Ball, dtMs: number, robotSpeedCmS = 0): void {
  const dt = dtMs / 1000;
  ...
  for (const opp of Object.values(world.opponents) as OpponentRobot[]) {
...
export function applyPushFromCircle(ball: Ball, center: Vector2, radius: number, strikerSpeedCmS: number): void {
 ...
 */
export function dohyoBorderADC(world: WorldModel, point: Vector2): number {
  if (world.arena.shape !== "circle") return 0;
...
export function isOutsideDohyo(world: WorldModel, point: Vector2): boolean {
  if (world.arena.shape !== "circle") return false;
  const center = { x: world.arena.width / 2, y: world.arena.height / 2 };
 ...
 */
export function isFullyInsideZone(
  position: Vector2,
  zone: { x: number; y: number; w: number; h: number },
  robotRadiusCm = ROBOT_RADIUS_CM,
): boolean {
  return (
    position.x - robotRadiusCm >= zone.x &&
 ...
 */
export function raycastDistance(world: WorldModel, origin: Vector2, angleRad: number, maxRangeCm = 80): number {
  const dirX = Math.cos(angleRad);
  const dirY = Math.sin(angleRad);
  let closest = maxRangeCm;

  const circularHit = (center: Vector2, radius: number): number | null => {
    const toX = center.x - origin.x, toY = center.y - origin.y;
    const proj = toX * dirX + toY * dirY;
    if (proj < 0 || proj > maxRangeCm) return null;
...
function raySegmentIntersection(
  ...
  if (t >= 0 && t <= maxRangeCm && u >= 0 && u <= 1) return t;
  return null;
}
 ...
 */
export function distanceToSharpADC(distanceCm: number, maxRangeCm = 80): number {
  if (distanceCm >= maxRangeCm) return 200;
