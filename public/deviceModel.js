...
export function devicesUsedByProgram(opcodes) {
 ...
 */
export function detectPinConflicts(devices) {
    const byPin = new Map();
    ...
    for (const [pin, ds] of byPin) {
        ...
        if (distinctResources.size > 1)
            conflicts.push({ pin, devices: ds.map((d) => d.name) });
    }
    return conflicts;
}
